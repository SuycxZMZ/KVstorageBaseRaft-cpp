...
- 目前bug已经定位到发送RPC请求上，起3个server节点模拟选举并发送心跳包，gdb追踪三个子进程，其中挂掉的进程死在send系统调用上，也就是rpc请求序列化完成之后的send系统调用，收到`SIGPIPE`信号，本地套接字或者对端被关闭。但是在gdb模式下，其他两个进程还没挂，使用nc连接其他两个server，还可以连接成功，说明并不是对端挂掉导致接收`SIGPIPE`信号，而是本地套接字被关闭，导致发送失败。本地关闭的原因正在查，比较离谱，因为都是强指针，只要server还在，基本不可能释放。
